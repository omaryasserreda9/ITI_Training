    <?php
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            try {
                ...
                $sql = "INSERT INTO information (name, email, password, room_number, picture_location)
                        VALUES (:name, :email, :password, :roomNumber, :pictureLocation)";
                $stmt = $conn->prepare($sql);
                $stmt->bindParam(':name', $name);
                $stmt->bindParam(':email', $email);
                $stmt->bindParam(':password', $password);
                $stmt->bindParam(':roomNumber', $roomNo);
                $stmt->bindParam(':pictureLocation', $file_name);
                $stmt->execute();
